...
from style_utils import read_and_resize_image
...
def _init_scene_images(trainer):
    ...
    viewpoint_stack = trainer.trainer.scene.getTrainCameras()
    ...
    for _, view in enumerate(viewpoint_stack):
        depth_image = trainer.trainer.get_render_pkgs(view)["depth"]
    ...
    trainer.ctx.depth_images = torch.stack(trainer.ctx.depth_images).to(device=trainer.trainer.device)
    trainer.ctx.original_images = torch.stack(trainer.ctx.original_images).to(device=trainer.trainer.device)
